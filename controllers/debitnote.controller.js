  ...
  try {
    ...
    if (update && id) {
      const update = await debitNoteModel.updateOne({ _id: id }, {
        $set: {
          party, debitNoteNumber, debitNoteDate, items, discountType, discountAmount, discountPercentage,
          additionalCharge, note, terms, finalAmount, autoRoundOff, roundOffAmount, roundOffType
        }
      })
    ...
    const insert = await debitNoteModel.create({
      userId: getUserData._id, companyId: getUserData.activeCompany, party, debitNoteNumber, debitNoteDate,
      items, discountType, discountAmount, discountPercentage, additionalCharge, note, terms, finalAmount,
      autoRoundOff, roundOffAmount, roundOffType
    });
