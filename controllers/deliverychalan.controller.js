...
const add = async (req, res) => {
  const {
    ...
    discountPercentage, additionalCharge, note, terms, update, id, finalAmount, autoRoundOff,
  ...
  try {
    ...
    if (update && id) {
      const update = await deliverChalanModel.updateOne({ _id: id }, {
        $set: {
          party, chalanNumber, chalanDate, validDate, items, discountType, discountAmount,
    ...
    const insert = await deliverChalanModel.create({
      ...
      party, chalanNumber, chalanDate, validDate, items, discountType,
      discountAmount, discountPercentage, additionalCharge, note, terms,
