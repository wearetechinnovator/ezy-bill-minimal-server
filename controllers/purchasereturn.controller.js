...
const add = async (req, res) => {
  const {
    ...
    autoRoundOff, roundOffAmount, roundOffType, returnInvoiceNumber
  } = req.body;
  ...
  try {
    ...
    if (update && id) {
      const update = await purchaseReturnModel.updateOne({ _id: id }, {
        $set: {
          party, purchaseReturnNumber, returnDate, items, discountType, discountAmount,
          discountPercentage, additionalCharge, note, terms, finalAmount, autoRoundOff,
          roundOffAmount, roundOffType, returnInvoiceNumber
        }
      })
    ...
    const insert = await purchaseReturnModel.create({
      ...
      returnDate, items, discountType, discountAmount, discountPercentage, additionalCharge, note,
      terms, finalAmount, autoRoundOff, roundOffAmount, roundOffType, returnInvoiceNumber
    });
